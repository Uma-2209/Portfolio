const PORTFOLIO_DATA = {
  ...
  "skills": [
    ...
    },
    {
      ...
      "icon": "fa-tools",
      "items": ["Microsoft Excel", "Canva", "PowerPoint", "Perplexity AI", "AI Tools for Finance", "Dashboard Reporting", "Data Visualization", "Python", "Streamlit"]
    }
  ],
  "projects": [
    {
      ...
      "description": "Conducted a comprehensive Q4 FY26 analysis monitoring macro-critical variables and supply chain health following Strait of Hormuz disruptions. Key finding: every $10/bbl rise widens India's CAD by ~0.5% of GDP. Identified Aviation and Paints as most vulnerable sectors.",
    ...
    {
      "id": 5,
      "title": "AI-Powered Cash Flow Forecasting & Risk Alert System",
      "category": "FP&A | Financial Planning & Analysis",
  ...
  "education": [
    ...
    },
    {
      ...
      "period": "Graduated",
      "status": "Completed",
      "description": "Studied core commerce subjects including financial accounting, business economics, corporate law, and taxation."
    },
    {
      "degree": "MA Sociology",
...
function buildMeta(meta, hero) {
  ...
  const md = document.querySelector('#meta-desc');
  if(md) md.setAttribute('content', meta.siteDescription);
}
...
function buildHero(hero, meta) {
  ...
  if (resumeBtn && meta.resumeUrl) {
    resumeBtn.href = meta.resumeUrl;
    resumeBtn.setAttribute('download', 'Uma_Bhargavi_Resume.pdf');
  }

  const navResume = document.querySelector('#nav-resume');
  if (navResume && meta.resumeUrl) {
    navResume.href = meta.resumeUrl;
    navResume.setAttribute('download', 'Uma_Bhargavi_Resume.pdf');
  }
...
function buildAbout(about) {
  const paraContainer = document.querySelector('#about-paragraphs');
  if(paraContainer) {
  ...
  const grid = document.querySelector('#about-highlights');
  if(grid) {
...
function buildProjects(projects) {
  ...
  projects.forEach((p, i) => {
    ...
    const imgContent = p.image
      ...
      : `<div style="display:flex;align-items:center;justify-content:center;height:100%;"><i class="fa fa-chart-pie" style="font-size:3rem;color:var(--gold);opacity:0.5"></i></div>`;
    const linkBtn = p.link
      ? `<a href="${p.link}" target="_blank" rel="noopener noreferrer" class="project-link-btn" style="display:inline-flex;align-items:center;gap:6px;padding:8px 16px;background:var(--gold);color:var(--navy);border-radius:6px;font-size:13px;font-weight:600;text-decoration:none;">View Project <i class="fa fa-arrow-right"></i></a>`
      : `<span style="opacity:0.4;font-size:13px;">Coming Soon</span>`;

    grid.innerHTML += `
      <div class="project-card reveal">
        <div class="project-img" style="height:220px;overflow:hidden;background:var(--navy-card);">${imgContent}</div>
        <div class="project-body" style="padding:24px;">
          ...
          <div class="project-tags" style="margin-top:6px;">${skillTags}</div>
          <div style="margin-top:16px;">${linkBtn}</div>
        </div>
      </div>`;
  });
}
...
function buildContact(contact) {
  ...
  if(!cards) return;
  const items = [
    { icon: 'fa-envelope', label: 'Email', val: `<a href="mailto:${contact.email}" style="color:var(--white)">${contact.email}</a>`, fab: false },
    { icon: 'fa-linkedin', label: 'LinkedIn', val: `<a href="${contact.linkedin}" target="_blank" style="color:var(--white)">View Profile</a>`, fab: true },
    { icon: 'fa-map-marker-alt', label: 'Location', val: contact.location, fab: false }
  ];
  items.forEach(item => {
  ...
  if(form && contact.formspreeId && contact.formspreeId !== 'YOUR_FORMSPREE_ID') {
...
function buildFooter(footer, contact) {
  ...
  const fl = document.querySelector('#footer-linkedin');
  if(fl) fl.href = contact.linkedin;
  const fe = document.querySelector('#footer-email');
  if(fe) fe.href = `mailto:${contact.email}`;
